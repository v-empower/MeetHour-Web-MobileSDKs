...
def schedulemeeting():
    ...
    # /instant meeting
        if request.method == "POST":
            ...
            if instant_meeting is not None and instant_meeting == 'true':
                ...
                schedule_meeting_object = ScheduleMeetingType.ScheduleMeeting("Instant  Meeting", "password", datetime.datetime.now().strftime(
                    '%I:%M'), 'PM', datetime.datetime.now().strftime('%d-%m-%Y'), timezone, is_show_portal=1, send_calendar_invite=1)
    ...
    # /Schedule meeting
                ...
                meeting_time = datetime.datetime.strptime(
                    request.form.get("meeting_time"), '%H:%M').strftime('%I:%M')
...
def join_meeting():
    ...
    try:
        if request.method == "POST":
            ...
            apiservice = apiServices.MHApiService()
            jwt_response = apiservice.generate_jwt(
                access_token, generate_jwt_object)
