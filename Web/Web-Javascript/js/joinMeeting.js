const generateJwtToken = async (id) => {
  try {
    const body = {
      meeting_id: window.location.search?.split("&")[0]?.split("?")[1]?.split("=")[1],
      contact_id: id,
    };
    ...
    var domain = "meethour.io";
        var options = {
            roomName: window.location.search?.split("&")[0]?.split("?")[1]?.split("=")[1], //Change to your Meeting ID
...
const viewMeeting = async () => {
  ...
  try {
    ...
    const id = window.location.search?.split("&")[0]?.split("?")[1]?.split("=")[1] || "";
    ...
    const text = document.createTextNode(meetingBody.organizer.name + "(Organizer / Account Owner");
    h1Tag.appendChild(text);
    divTag.appendChild(h1Tag);
    divTag.setAttribute('onclick', `generateJwtToken(${meetingBody.organizer.id})`)
    divTag.setAttribute('class', 'flex my-4 gap-2 px-4 py-2 border cursor-pointer text-black hover:text-white hover:bg-blue-600 border-blue-600 rounded-md');
    ...
    const divTag = document.createElement('div');
    const h1Tag = document.createElement('h1');
    const text = document.createTextNode(host.first_name + "(Moderator / Host)");
    h1Tag.appendChild(text);
    divTag.appendChild(h1Tag);
