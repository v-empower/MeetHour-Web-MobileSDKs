async function getUser() {
  ...
  async function getContactsList() {
    ...
    body
        ...
        response.contacts.forEach((contact) => {
          ...
          const divText = document.createTextNode(contact.first_name + " (" + contact.email + ") ");
          ...
          newDiv.setAttribute('class', 'participant text-gray-700 block px-4 py-2 text-sm cursor-pointer');
          const participantsDropdown = document.querySelector('#participants-dropdown');
        ...
        response.contacts.forEach((contact) => {
          ...
          newDiv.setAttribute('name', contact.id);
          newDiv.setAttribute('class', 'moderator text-gray-700 block px-4 py-2 text-sm cursor-pointer');
  ...
  function displaySelectedParticipants(name) {
    const newDiv = document.createElement('div');
      ...
      appendChild(`<div class="flex justify-between"><h1>${name}</h1><button id='remove-button' class="border border-slate-400 rounded-md px-3">Remove</button></div>`)
  }
