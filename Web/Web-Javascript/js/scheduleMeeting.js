async function getUser() {
  const response =
      await ApiServices.userDetails(localStorage.getItem("accessToken") || "");
...
async function instantMeeting(body) {
  try {
    const response = await ApiServices.scheduleMeeting(
        localStorage.getItem("accessToken") || "", body);
...
async function getTimezones() {
  const response =
      await ApiServices.timezone(localStorage.getItem("accessToken") || "")
...
async function getContactsList() {
  ...
  try {
    ...
    response.contacts.forEach((contact) => {
      ...
      const divText = document.createTextNode(contact.first_name + " (" +
                                              contact.email + ") ");
      ...
      const participantsDropdown =
          document.querySelector('#participants-dropdown');
...
function displaySelectedParticipants(name) {
  ...
  appendChild(`<div class="flex justify-between"><h1>${
      name}</h1><button id='remove-button' class="border border-slate-400 rounded-md px-3">Remove</button></div>`)
}
...
const removeParticipant = (participant) => {
  ...
  existing.splice(existing.indexOf(participant),
