...
async function getContactsList(contacts) {
  try {
    ...
    response?.contacts?.forEach((contact) => {
      ...
      const optionText = document.createTextNode(contact.first_name + " (" +
                                                 contact.email + ") ");
      ...
      const participantsDropdown =
          document.querySelector('#mySelectParticipants');
...
const removeParticipant = (event) => {
  ...
  existing.splice(existing.indexOf(event.target.id),
  ...
  document.getElementById('attendArray').value =
      JSON.stringify(localStorage.getItem("attendees"));
};
...
const removeModerator = (event) => {
  ...
  document.getElementById('hostusersArray').value =
      JSON.stringify(localStorage.getItem("hostusers"));
};
