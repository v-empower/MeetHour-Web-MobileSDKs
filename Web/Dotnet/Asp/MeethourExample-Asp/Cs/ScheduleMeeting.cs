...
public class ScheduleMeetingController : Controller {
  ...
  ScheduleMeeting() // Change the return type to Task<IActionResult>
  {
    bool success = false;
    bool error = false;
    string? message = null;
    ...
    try {
      ...
      timezone_array = System.Text.Json.JsonSerializer.Deserialize<object>(
          timezoneResponse.timezones);

      var methodhttp = Request.Method;
      var contactsObject = new Contact(0, 0, 0);
      apiService = new MHApiService();
      contactsResponse = await apiService.Contact<ContactResponse>(
          accessToken, contactsObject); // Await the Contact API call

      contact_array = System.Text.Json.JsonSerializer.Deserialize<object>(
      ...
      if (Request.Method == "POST") {
        ...
        if (Request.Method == "POST" && !string.IsNullOrEmpty(instantMeeting) &&
            instantMeeting == "true") {

          var scheduleMeetingObject = new ScheduleMeeting(
              "Instant Meeting", "123456789", "Asia/Kolkata",
              DateTime.Now.ToString("dd-MM-yyyy"),
              DateTime.Now.ToString("hh:mm"), "PM", 1, true, "Local");
          apiService = new MHApiService();
          var response =
              await apiService.ScheduleMeeting<ScheduleMeetingResponse>(
                  accessToken,
                  scheduleMeetingObject); // Await the ScheduleMeeting API call
        ...
        else if (!string.IsNullOrEmpty(scheduleMeeting) &&
          ...
          var meetingTime =
              DateTime
                  .ParseExact(Request.Form["meeting_time"], "HH:mm",
          ...
          if (!string.IsNullOrEmpty(Request.Form["mySelectParticipants"]) &&
              Request.Form["mySelectParticipants"] != "0") {
            mySelectParticipants.Add(Request.Form["mySelectParticipants"]);
          }

          var mySelectModerators = new List<string>();
          if (!string.IsNullOrEmpty(Request.Form["mySelectModerators"]) &&
              Request.Form["mySelectModerators"] != "0") {
          ...
          var scheduleMeetingObject = new ScheduleMeeting(
              ...
              attend: mySelectParticipants, hostUsers: mySelectModerators,
              options: new[] { "ALLOW_GUEST" },
              defaultRecordingStorage: "Local");
          apiService = new MHApiService();
          var ScheduleResponse =
              apiService.ScheduleMeeting<ScheduleMeetingResponse>(
                  accessToken, scheduleMeetingObject);
    ...
    var combinedViewModel = new CombinedViewModel {
      ...
      MeetingModel = MeetingModel, contactModel = contactModel,
