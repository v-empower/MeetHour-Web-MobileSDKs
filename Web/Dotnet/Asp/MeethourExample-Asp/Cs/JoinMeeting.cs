...
namespace JoinMeeting.Controllers
{
    ...
    public class JoinMeetingController : Controller
    {
        ...
        public JoinMeetingController(IMemoryCache memoryCache, ILogger<JoinMeetingController> logger)
        {
        ...
        public async Task<IActionResult> JoinMeeting(string meeting_id, string pcode)
        {
            ...
            try
            {
                ...
                if (AccessToken == null)
                {
                    throw new Exception("Access token is null. Please ensure you have a valid access token.");
                }
                ...
                if (Request.Method == "POST")
                {
                    ...
                    var JwtResponse = await apiService.GenerateJwt<GenerateJwtResponse>(AccessToken, generateJwtObject);
                ...
                else if (Request.Method == "GET")
                {
                    if (!string.IsNullOrEmpty(meeting_id))
                    {
                        ...
                        var ViewMeetingResponse = await apiService.ViewMeeting<ViewMeetingResponse>(AccessToken, viewMeetingsObject);

                        var responseJson = JsonSerializer.Serialize(ViewMeetingResponse);
                        _logger.LogInformation("Serialized ViewMeetingResponse: {responseJson}", responseJson);
                        ...
                        using (JsonDocument doc = JsonDocument.Parse(responseJson))
                        {
                            ...
                            if (meetingElement.TryGetProperty("pcode", out JsonElement pcodeProperty))
                            {
                                pcode = pcodeProperty.ToString();
                            }
                            else
                            {
                                _logger.LogError("pcode not found in ViewMeetingResponse. JSON: {responseJson}");
                            }
                            ...
                            if (meetingElement.TryGetProperty("hosts", out var hostsProperty))
                            {
                                ...
                                try
                                {                                    
                                    if(hostsProperty.ValueKind== System.Text.Json.JsonValueKind.Array)
                                    {
                                    var users = JsonSerializer.Deserialize<List<object>>(hostsProperty.ToString());
                                    var b = users[0];
                                    var yourObject = System.Text.Json.JsonDocument.Parse(b.ToString());
                                   ...
                                   // hosts = sb.ToString();
                                    ...
                                    {
                                        if (root.TryGetProperty("organizer", out JsonElement organizersProperty))
                                        {
                                            organizer = organizersProperty; //organizersProperty.ToString();

                                        }
                                        else
                                        {
                                            _logger.LogError("organizers not found or not a string in ViewMeetingResponse. JSON: {responseJson}");
                                        }
                                    }
                                ...
                                {
                                    _logger.LogError(ex, "Error deserializing hosts. JSON: {responseJson}");
                                }
                            }
                            else
                            {
                                _logger.LogError("hosts not found in ViewMeetingResponse. JSON: {responseJson}");
                            }

                            if (root.TryGetProperty("meeting_attendees", out var attendeeProperty))
                            {
                                try
                                {
                                    if (attendeeProperty.ValueKind == System.Text.Json.JsonValueKind.Array)
                                    {
                                        var users = JsonSerializer.Deserialize<List<object>>(attendeeProperty.ToString());
                                        var b = users[0];
                                        var yourObject = System.Text.Json.JsonDocument.Parse(b.ToString());
                                        var id = yourObject.RootElement.GetProperty("meeting_attendee_id");
                                        var name = yourObject.RootElement.GetProperty("first_name");
                                        var email = yourObject.RootElement.GetProperty("email");
                                ...
                                {
                                    _logger.LogError(ex, "Error deserializing meeting_attendees. JSON: {responseJson}");
                                }
                            }
                            else
                            {
                                _logger.LogError("meeting_attendees not found in ViewMeetingResponse. JSON: {responseJson}");
                            }
                            if (root.TryGetProperty("organizer", out JsonElement organizerProperty))
                            {
                            ...
                            {
                                _logger.LogError("organizer not found or not a string in ViewMeetingResponse. JSON: {responseJson}");
                            }
                            

                            // Assign values to meeting_attendees only if all parts are available
                            if (organizer != null || hosts != null || attendee != null)
                            {
                                meeting_attendees = new
                                {
                                    organizer,
                                    hosts,
                                    attendee
                            ...
                            {
                                _logger.LogError("One or more required properties (organizer, hosts, meeting_attendees) are missing or invalid.");
                            }
                        }
                    }
                }
