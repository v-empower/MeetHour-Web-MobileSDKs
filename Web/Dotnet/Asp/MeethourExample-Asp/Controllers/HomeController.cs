...
namespace MeethourExample_Asp.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(Index index, IMemoryCache memoryCache, ILogger<HomeController> logger)
        {
        ...
        public async Task<IActionResult> IndexAsync()
        {
            ...
            try
            {
                if (Request.Method == "POST")
                {
                    ...
                    if (getAccessTokenValue == "true")
                    {
                        ...
                        if (accessToken != null)
                        {
                            _memoryCache.Set("access_token", accessToken, TimeSpan.FromMinutes(30));
            ...
            {
                // Log the exception
                _logger.LogError(e, "An error occurred while processing the request in the Index method.");
            ...
            {
                return View("~/Views/Home/Index.cshtml", combinedViewModel);  // Pass the model to the view
            }
            else
            {
