...
/** MeetHourPlugin */
public class MeetHourPlugin() : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    // The EventChannel for broadcasting MeetHourEvents to Flutter
    ...
    override fun onAttachedToEngine(@NonNull flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
        channel = MethodChannel(flutterPluginBinding.binaryMessenger, MeetHour_METHOD_CHANNEL)
        channel.setMethodCallHandler(this)

        eventChannel = EventChannel(flutterPluginBinding.binaryMessenger, MeetHour_EVENT_CHANNEL)
        eventChannel.setStreamHandler(MeetHourEventStreamHandler.instance)
    }
    ...
    companion object {
        ...
        fun registerWith(registrar: Registrar) {
            val plugin = MeetHourPlugin(registrar.activity())
            val channel = MethodChannel(registrar.messenger(), MEETHOUR_METHOD_CHANNEL)
            channel.setMethodCallHandler(plugin)


            val eventChannel = EventChannel(registrar.messenger(), MEETHOUR_EVENT_CHANNEL)
            eventChannel.setStreamHandler(MeetHourEventStreamHandler.instance)
        }

        const val MEETHOUR_PLUGIN_TAG = "MEETHOUR_MEET_PLUGIN"
        const val MEETHOUR_METHOD_CHANNEL = "meet_hour"
        const val MEETHOUR_EVENT_CHANNEL = "meet_hour_events"
        const val MEETHOUR_MEETING_CLOSE = "MEETHOUR_MEETING_CLOSE"
    }
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        Log.d(MEETHOUR_PLUGIN_TAG, "method: ${call.method}")
        Log.d(MEETHOUR_PLUGIN_TAG, "arguments: ${call.arguments}")
    ...
    private fun joinMeeting(call: MethodCall, result: Result) {
        ...
        Log.d(MEETHOUR_PLUGIN_TAG, "Joining Room: $room")

        val userInfo = MeetHourUserInfo()
        ...
        if (serverURLString == null) {
            serverURLString = "https://meethour.io";
        }
        val serverURL = URL(serverURLString)
        Log.d(MEETHOUR_PLUGIN_TAG, "Server URL: $serverURL, $serverURLString")

        val optionsBuilder = MeetHourConferenceOptions.Builder()
